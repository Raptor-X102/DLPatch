...
struct LibraryInfo {
    ...
    std::vector<std::pair<uintptr_t, uintptr_t>> segments; // Memory segments [start, end)
};
...
struct TrackedLibrary {
    ...
    TrackedLibrary(const std::string& p, uintptr_t h, uintptr_t addr, const std::vector<std::string>& functions)
...
public:
    ...
    DL_Manager(pid_t pid) : pid_(pid), dlopen_addr_(0), dlclose_addr_(0), syscall_insn_(0) {
        init_addresses();
    }
    ...
    //-------------------------------------------------------------------------
    std::vector<std::string> get_loaded_libraries() const;      // Get list of loaded libraries
    LibraryInfo get_library_info(const std::string& lib_name) const; // Get info about specific library
    void print_loaded_libraries() const;                        // Print all loaded libraries
    
    //-------------------------------------------------------------------------
    // Safety checks (DL_Manager_check.ipp)
    //-------------------------------------------------------------------------
    bool is_safe_to_replace(const std::string& lib_name) const; // Check if library can be safely replaced
    ...
    bool rollback_function(const std::string& lib_path, const std::string& func_name); // Rollback single function
    
    //-------------------------------------------------------------------------
    // Architecture layer helpers (declaration only - implemented in DL_Manager_replace.ipp)
    //-------------------------------------------------------------------------
...
private:
    ...
    //-------------------------------------------------------------------------
    uintptr_t find_syscall_instruction(uintptr_t libc_base); // Find syscall instruction in libc
